...
def main():
    ...
    model = layers.LinearClassifier(in_features=2048, num_classes=10)
    model.to(device)
    ...
    criterion = losses.TraditionalCELoss()
    
    for epoch in tqdm.tqdm(range(args.epochs)):
        train_loss = utils.train_one_epoch(model, criterion, optimizer, train_dataloader)
        val_loss = utils.evaluate(model, criterion, val_dataloader)
    ...
    train_results = utils.get_predictions_and_accuracy(
        train_dataloader, model, num_samples=None, device=device)
    val_results = utils.get_predictions_and_accuracy(
        val_dataloader, model, num_samples=None, device=device)
    test_results = utils.get_predictions_and_accuracy(
        test_dataloader, model, num_samples=None, device=device)
    ood_results = utils.get_predictions_and_accuracy(
        ood_dataloader, model, num_samples=None, device=device)
